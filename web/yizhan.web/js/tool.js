 var tool = (function () {
        ...
        //关闭等待动画
        this.delWait = function (d) {
            d.close().remove();
        }

        //模态对窗口
        this.modalDialog = function (title, form, okCallback, cancelCallback, okbtn, cancelBtn) {
            dialog({
                title: title,
                content: form,
                okValue: okbtn || '确 定',
                ok: okCallback || function () { },
                cancelValue: cancelBtn || '取 消',
        ...
        this.initForm = function ($form) {
            ...
            //限制格式
            $form.find('[reg-exp]').keyup(function () {
                var $that = $(this);
                var re = new RegExp($that.attr('reg-exp'));
                var val = $that.val();
                if (val == '' || re.exec(val)) {
                    $that.attr(preattr, val);
                } else {
                    $that.val($that.attr(preattr));
                }
            });
